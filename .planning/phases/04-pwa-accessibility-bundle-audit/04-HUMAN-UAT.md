...
[item 1 PASSED on first push; item 2 awaits real-device testing]

## Tests

### 1. Lighthouse PWA score ≥ 0.90 on deployed URL (PWA-05)
expected: After pushing to `main`, the GitHub Actions `lighthouse` job runs `lighthouse <pages-url> --only-categories=pwa --chrome-flags="--headless"` and the resulting `categories.pwa.score` is ≥ 0.90. Score of 1.00 is typical for a properly configured PWA.
how_to_test: Push a commit to `main` (or trigger the workflow manually). Watch the `lighthouse` job in the Actions tab. The job fails with a clear message if the score is below the gate, succeeds otherwise.
result: PASSED — Lighthouse PWA score 1.00 (100%) on first successful CI run (workflow run 25092302358, 2026-04-29). Live at https://matwming.github.io/flappy-3d/. Gate: ≥0.90 — exceeded by full margin.
